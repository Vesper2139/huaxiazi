...
> 历史审计记录（2026-09-06）。本文用于追溯整改，不替代当前隐私说明；当前边界见 [隐私与数据](隐私与数据.md)。
...
| P1 | 1 | 旧 Release 附件仍未替换为本轮安全加固构建 |
| P2 | 3 | Git 提交者个人邮箱、内部审计绝对路径、运行数据持久化边界需要治理 |
| P3 | 1 | 部分测试与开源归属信息仍可进一步最小化 |

最终判定：**NOT PRIVACY CLEAN**。

原因不是在源码中发现了新的明文密钥，而是：已有密钥曾被暴露、Git 历史含个人联系方式，且旧发布物没有包含最新安全修复。安全加固代码和本报告已同步到 GitHub `main`；现有 Release 附件仍待签名构建后替换。
...
| PRIV-002 | P1 | GitHub Release v2.0.0 附件 | 发布不同步 | 当前 `main` 已包含 DPAPI、日志、更新校验和测试加固，但现有附件仍来自旧构建 | 使用干净工作树完成签名构建、重新上传附件并复核 SHA-256 |
...
放行条件：撤销并轮换已暴露 API Key；使用当前 `main` 完成签名构建并替换旧附件；决定是否批准 Git 历史邮箱清理。完成后重新执行本报告中的二次扫描，才能改判为 `PRIVACY CLEAN`。
