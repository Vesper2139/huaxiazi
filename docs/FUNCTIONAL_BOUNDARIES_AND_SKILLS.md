...
> 本文聚焦 Skill 生命周期与功能边界；完整的 Agent 分层、上下文传递和工具编排见 [Agent 架构与工程编排](AGENT_ARCHITECTURE.md)。

## 第一性原则

话匣子的核心价值不是暴露更多开关，而是用最少操作把用户原始表达转换为可直接使用的结果。系统必须先保证事实、立场、隐私和输出协议，再允许个性化或 Skill 改变表达方法。
...
| Skill 运行选择 | `ExpressionSkillRouter` | 不执行脚本、不读取密钥、不决定安全规则 | 启停是唯一运行状态；每次最多组合三个高置信、无冲突的匹配能力 |
...
- 运行投影会排除许可证正文、工具读取、多阶段审批和与话匣子输出协议冲突的章节，导出仍保留完整快照。
- 所有包只读取允许的文本参考文件；脚本、Shell、Python、MCP、浏览器和任意文件访问仍然禁止。

## 普通用户与深度用户

普通用户无需管理 Skill；预置能力默认启用并自动匹配。深度用户可在同一“表达能力”页面启停、导入、导出或从只读预置派生可编辑副本。不存在全局开关，也不暴露底层安全策略或任意工具权限；运行时会记录实际选中的 Skill 及权重，冲突时回退到主 Skill。
