...
public class SettingsViewModelTests
{
    ...
    public async Task ToggleSelectedSkill_ImmediatelyPersistsItsEnabledState()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), "VesperSkillSettings_" + Guid.NewGuid().ToString("N"));
        try
        {
    ...
    {
        var original = App.Settings;
        var root = Path.Combine(Path.GetTempPath(), "VesperDataPath_" + Guid.NewGuid().ToString("N"));
