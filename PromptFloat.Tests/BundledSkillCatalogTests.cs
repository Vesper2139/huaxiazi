...
{
    private readonly string _installed = Path.Combine(Path.GetTempPath(), "VesperBundledSkills_" + Guid.NewGuid().ToString("N"));
    ...
    [Fact]
    public void CatalogUpgrade_RetiresOnlyUnmodifiedLegacyPresetsAndKeepsUserCopies()
    {
        ...
        File.WriteAllText(Path.Combine(legacySkill, "SKILL.md"), """
            ---
            name: text-polisher
            description: Legacy Vesper preset.
            metadata:
              vesper.modes: "polish"
            ---
            Preserve facts.
        ...
        Assert.DoesNotContain(records, item => item.Id == "text-polisher" && item.Source == AgentSkillSource.Preset);
