...
///
/// 通过反射把静态配置路径重定向到临时目录，避免污染用户真实 %AppData%\PromptFloat\config.json。
/// </summary>
public class ConfigServiceTests
{
    private static string MakeTempConfigDir()
    {
        var dir = Path.Combine(Path.GetTempPath(), "PromptFloatCfg_" + Guid.NewGuid().ToString("N"));
  ...
  ""windowTop"": NaN
    ...
    [Fact]
    public void Load_WhenNewLocationIsEmpty_ImportsOldPromptFloatConfigAndSanitizesIt()
    {
        var root = MakeTempConfigDir();
        var newDir = Path.Combine(root, "new");
        var oldDir = Path.Combine(root, "old", "PromptFloat");
