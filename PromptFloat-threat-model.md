...
> 全量审计记录。当前架构、数据边界与发布要求以 `docs/AGENT_ARCHITECTURE.md`、`docs/隐私与数据.md` 和 `docs/发布检查清单.md` 为准。

> **发布标准更新（v2.0.1）：**本审计保留当时的风险快照。对于当前的小型开源桌面工具，代码签名是可选增强项，不再作为公开源码或发布无签名二进制的硬性前提；无签名构件必须显式构建、通过 CI、来自干净标签、附带 SHA-256 校验值，并在发布页披露 Windows“未知发布者”/SmartScreen 提示。
